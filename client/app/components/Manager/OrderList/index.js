...
import DownloadInvoiceButton from '../../Common/DownloadInvoiceButton'; // ✅ IF you're in Manager/OrderList
...
const OrderList = props => {
  ...
  const renderFirstItem = order => {
    if (order.products) {
      ...
      return <img className='item-image' src='/images/placeholder-image.png' alt='Product' />;
    }
  };
    ...
    <div className='order-list'>
      {orders.map((order, index) => (
        <div key={index} className='order-box'>
              ...
              <div className='d-flex flex-column flex-xl-row justify-content-between flex-1 ml-lg-2 mr-xl-4 p-3'>
                <div className='order-details'>
                  ...
                  <div className='mb-1'>
                    ...
                    <span className='order-label'>{` $${order?.totalWithTax || 0}`}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* ✅ Download invoice button outside the <Link> */}
          <div className="invoice-download px-3 pb-3">
